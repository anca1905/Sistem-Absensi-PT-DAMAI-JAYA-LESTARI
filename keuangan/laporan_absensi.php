...
<style>
    ...
    .filter-input:focus, .filter-select:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,.12); }
    ...
    .table-absen tbody tr:hover td { background: #f0fdf4; }
    .table-absen td.td-name { text-align: left; font-weight: 700; color: #064e3b; min-width: 150px; }
    .table-absen td.td-nik  { font-size: 11px; color: #6b7280; }
    ...
    .b-h { background: #d1fae5; color: #065f46; }
    ...
    .b-c { background: #ffedd5; color: #9a3412; }

    .summary-total { font-weight: 800; color: #065f46; }
    .afdeling-row td { background: #f0fdf4; font-weight: 700; font-size: 11px; color: #065f46; border-top: 2px solid #d1fae5; }
...
<div class="print-area">
        ...
        <table class="table-absen">
            ...
            foreach ($list_karyawan as $kar):
                if ($cur_afdeling !== $kar['afdeling']) {
                    ...
                    echo "<tr class='afdeling-row'><td colspan='" . ($jumlah_hari + 7) . "'>📍 Afdeling: " . htmlspecialchars($cur_afdeling ?: '-') . "</td></tr>";
                }
            ...
            <tr>
                ...
                <?php for($i=1; $i<=$jumlah_hari; $i++): 
                    $s = $absen[$i] ?? null;
                    if ($s == 'hadir' || $s == 'tepat_waktu') { $h++; echo '<td><span class="badge b-h">H</span></td>'; }
                    elseif ($s == 'terlambat') { $h++; echo '<td><span class="badge b-t">T</span></td>'; }
                    elseif (in_array($s, ['alpha','alpa','alfa'])) { $a++; echo '<td><span class="badge b-a">A</span></td>'; }
                    elseif ($s == 'izin') { $izin++; echo '<td><span class="badge b-i">I</span></td>'; }
                    elseif ($s == 'sakit') { $izin++; echo '<td><span class="badge b-s">S</span></td>'; }
                    elseif ($s == 'cuti') { echo '<td><span class="badge b-c">C</span></td>'; }
                    else { echo '<td><span style="color:#d1d5db;">-</span></td>'; }
                endfor; ?>
                <td class="summary-total"><?= $h ?></td>
                <td style="font-weight:700; color:#991b1b;"><?= $a ?></td>
                <td style="font-weight:700; color:#1e40af;"><?= $izin ?></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <div style="text-align:center; padding:48px; background:white; border-radius:14px; border:1px solid #e5e7eb; color:#9ca3af;">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="1.5" style="margin:0 auto 12px;display:block;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle></svg>
